...
options=("Inbox-API" "Inbox-Worker" "Socket" "test")
select_option "${options[@]}"
choice=$?
update_env_variable ".env.${NODE_ENV}" "SERVICE" "${options[$choice]}"
if [[ "$NODE_ENV" == "production" ]]; then
    node --import ./loader/loader-register.mjs scripts/start.js
else
    nodemon --import ./loader/loader-register.mjs scripts/start.js
fi